...
export default function CampaignDetail() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      // Chỉ cần lấy data Campaign vì Backend đã tính sẵn tiến độ trong đó rồi
      const campRes = await fetch(`${API_URL}/${id}`);
      const campResult = await campRes.json();
      
      // Lấy dữ liệu từ campResult.data (dựa theo cấu trúc API của bạn)
      setCampaign(campResult.data || campResult);
    } catch (error) {
  ...
  const handleDonate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setDonateAmount("");
        setDonorName(""); 
        // Load lại data để cập nhật con số tiến độ mới từ Backend
        await fetchData(); 
      } else {
  ...
  // --- THAY ĐỔI QUAN TRỌNG: Lấy dữ liệu đã tính sẵn từ Backend ---
            ...
            {status === 'COMPLETED' ? (
              <div className="mt-2 bg-slate-50 border border-slate-100 rounded-2xl p-6 text-center">
                ...
                <p className="text-sm text-slate-500">
                  {isGoalReached ? "Thank you! The goal has been achieved. 🎉" : "The time has expired. ⏰"}
                </p>
              </div>
            ) : (
              <form onSubmit={handleDonate} className="space-y-5 pt-2">
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-medium"
                />
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 font-bold text-xl"
                    required
                    min="1"
                  />
                </div>
                {message.content && (
                  <div className={`p-4 rounded-xl text-sm font-semibold ${message.type === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
                    {message.content}
                  </div>
                )}
                <button 
                  disabled={isSubmitting}
                  className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-700 active:scale-[0.98] transition-all shadow-xl shadow-emerald-200 flex items-center justify-center gap-3 disabled:opacity-50"
                >
                  {isSubmitting ? <Loader2 className="w-6 h-6 animate-spin" /> : <Heart className="w-6 h-6" />}
                  Donate Now
                </button>
              </form>
            )}
